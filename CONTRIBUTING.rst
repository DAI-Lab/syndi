...
2. The tests that cover a module called ``syndi_benchmark/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/syndi_benchmark/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_syndi_benchmark
...
2. Update the version in ``setup.cfg``, ``syndi_benchmark/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``syndi_benchmark/__init__.py``
...
dependency specification, either in ``setup.py``::
    ...
    install_requires = [
        ...
        'syndi-benchmark>=X.Y.Z.dev',
        ...
    ]

or in command line::

    pip install 'syndi-benchmark>=X.Y.Z.dev'


.. _GitHub issues page: https://github.com/DAI-Lab/syndi-benchmark/issues
.. _Build Status page: https://github.com/DAI-Lab/syndi-benchmark/actions
