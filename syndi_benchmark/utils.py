...
from sdv.tabular import CTGAN, TVAE, CopulaGAN, GaussianCopula  # Synthetic data
...
def split_data(dataset_path="data.csv", output_directory="data",
    ...
    train, test, target_train, target_test = train_test_split(
        data.drop(target_name, axis=1), data[target_name], test_size=0.4, random_state=42)
...
def create_default_generators(train_dataset="data/train.csv",
                              generators=SDV_GENGERATORS, output_directory="generators"):
